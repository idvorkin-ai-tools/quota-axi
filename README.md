...
- Only HTTP 401 is a definitive auth rejection. HTTP 403 is **not** sign-out: ElevenLabs keys carry scope restrictions (the subscription read needs the `user_read` permission) and an optional IP allowlist, either of which rejects this one operation while the key stays live. It is reported as `elevenlabs_user_read_denied` with `authStatus: usable`, and it preserves the cache.
...
- Because the key _is_ the account, ElevenLabs snapshots are cache-scoped by an opaque SHA-256 of the answering source plus a one-way digest of that key. A different key gets a different slot, so a rotated or second key never inherits the previous subscription's windows, and the key itself never enters the cache.
